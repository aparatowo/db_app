...
docker build . && docker run -env AWS_REGION=$(aws ssm get-parameters --names REGION_AWS --with-decryption) -env DB_SECRET=$(aws ssm get-parameters --names DB_SECRET --with-decryption) .